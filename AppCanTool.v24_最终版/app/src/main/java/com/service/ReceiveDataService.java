...
import com.Ctrol.CanData;
import com.Ctrol.CaninData;
import com.Ctrol.CtrolData;
...
public class ReceiveDataService extends Service {
	...
	private UB_BlueTooth mBlueTooth;
...
//						meData		=transData(meData);
	...
	/////////////////
	//	数据解析;
	private D_Data analData(D_Data meData, String param){
		if(param!=null&&!param.equals("")){			
			CanData cAnalData =new CanData(param);
	...
	//	数据插入库;
	private void inputData(Context context, long l1,long l2,String chn,String dir,D_Data meData){
		Log.i("MyLog", "contenxt="+context);
		CaninData cInputDataBase=new CaninData(context);
		cInputDataBase.inputMes(l1, l2, chn, dir, meData);
		
	}
	
	//	数据转码;
	private D_Data transData(Context context, D_Data meData){
		
		CtrolData cTransData=new CtrolData(context);
