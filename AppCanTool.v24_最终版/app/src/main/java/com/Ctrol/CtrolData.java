...
import com.AllView.device.D_Message;
import com.AllView.device.D_Signal;
import com.AllView.usebox.UB_DBHelper;

import android.annotation.SuppressLint;
import android.content.Context;

public class CtrolData extends D_Data {
	private D_Data meData;
	private UB_DBHelper mdbHelper;

	//	16进制数据包;
	private ArrayList<char[]>   datas16;
	private Map<Integer, String> datas2;
	//	信号信息列表;
	private ArrayList<D_Signal>  listSignals;
	

	public CtrolData(Context context) {
		//	数据库控制类;
		this.mdbHelper	=   new UB_DBHelper(context);
	}
	
	public D_Data getmData() {
		return meData;
	}
...
//		}
	...
	private void destory8Matrix(){
		 ...
		 *对其中的每一行的信息值进行重新的翻转;
		 * */
		for(D_Signal mSignal:listSignals){
			//	初始位置;
	...
	//	查询message大表中的id信息;
	private ArrayList<D_Signal> queryInfofromSignal(int id10){
		ArrayList<D_Signal>  listSignals	=new ArrayList<D_Signal>();
		D_Message mMessage	=null;
		String				sql;
		//	大类的sql语句;
		...
		for(String[] strs:listMesssage){
			mMessage=new D_Message(Integer.parseInt(strs[0]), strs[1], Integer.parseInt(strs[2]),strs[3], Integer.parseInt(strs[4]),strs[5]);
		}
		//	判断Signal表的信息;
		if(mMessage==null){
			return null;
		...
		for(String[] strs:listSignal){
			D_Signal mSignal=new D_Signal(Integer.parseInt(strs[0]),strs[1], strs[2], strs[3], strs[4], strs[5],strs[6], strs[7],Integer.parseInt(strs[8]));
